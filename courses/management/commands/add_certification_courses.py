"""
Django management command to add certification courses
Usage: python manage.py add_certification_courses
"""
...
class Command(BaseCommand):
    help = 'Add all 6 certification courses to the database'
    ...
    def handle(self, *args, **options):
        ...
        for course_data in courses_data:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f"✓ Created: {course_data['title']} (KES {course_data['price']})")
                )
                created_count += 1
            else:
                ...
                course.save()
                self.stdout.write(
                    self.style.WARNING(f"✓ Updated: {course_data['title']} (KES {course_data['price']})")
                )
                updated_count += 1
