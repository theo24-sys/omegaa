...
def on_payment_completed(sender, instance, created, update_fields, **kwargs):
    ...
    if user.user_type == 'househelp':
        ...
        # Case 2: Monthly contribution payment
        elif instance.contribution:
            logger.info(f"Recording contribution payment for {user.username}")
            ...
            contribution.save(update_fields=['payment_status', 'payment_date', 'mpesa_transaction_id'])
    ...
    # ─── EMPLOYER PAYMENTS (Subscription plans) ─────────────────────────────
        ...
        if instance.plan:
            ...
            if instance.plan.plan_type == 'profile_highlight':
                user.is_premium = True
                user.premium_until = expires_at
                user.save(update_fields=['is_premium', 'premium_until'])
                logger.info(f"Enabled premium profile for employer {user.username}")

            if instance.plan.plan_type in ['job_boost', 'featured_listing']:
                job = instance.job or user.jobs.filter(is_active=True).order_by('-created_at').first()
                if job:
                    job.is_featured = True
                    job.featured_until = expires_at
                    job.save(update_fields=['is_featured', 'featured_until'])
                    logger.info(f"Featured job '{job.title}' for employer {user.username}")
